...
def find_template(screen, template, threshold=0.85):
    """Ищет шаблон на готовом скриншоте.

    Параметры:
        screen (np.ndarray): BGR-картинка экрана (от grab_screen).
        template (np.ndarray): BGR-картинка шаблона (от cv2.imread).
        threshold (float): минимальная уверенность совпадения (0..1).

    Возвращает:
        tuple[int, int] | None: координаты центра (cx, cy) или None.
    """
    ...
    if th > sh or tw > sw:
        print(f"❌ Шаблон {tw}x{th} больше экрана {sw}x{sh}")
        return None
...
def wait_for_template(template, timeout=30.0, interval=1.0, threshold=0.85):
    """Ждёт появления шаблона на экране, периодически делая скриншот.

    Параметры:
        template (np.ndarray): BGR-картинка шаблона.
        timeout (float): сколько секунд ждать максимум.
        interval (float): пауза между попытками.
        threshold (float): минимальная уверенность совпадения.

    Возвращает:
        tuple[int, int] | None: координаты центра (cx, cy) или None, если не дождались.
    """
    start = time.time()
    attempts = 0
    ...
    while time.time() - start < timeout:
        attempts += 1
        screen = grab_screen()
        pos = find_template(screen, template, threshold)
        if pos is not None:
            print(f"✅ Найдено за {attempts} попыток ({time.time()-start:.1f}с)")
            return pos
        time.sleep(interval)

    print(f"⏰ Не дождались за {timeout}с")
    return None
